...
| 问题 | 文件 | 描述 | 状态 |
|------|------|------|------|
| API Key 明文存储 | ConfigManager.java | API Key 以明文存储在 YAML 文件中，无加密保护 | 待修复 |
| 并发安全问题 | AIResponseCache.java | LinkedHashMap 非线程安全，多线程访问可能导致数据不一致 | 待修复 |
| URL 验证缺失 | OpenAICompatibleProvider.java | 用户可配置任意 base_url，存在 SSRF 风险 | 待修复 |
...
| 任务 | 描述 | 预计工时 |
|------|------|----------|
| API Key 加密存储 | 使用 AES 加密存储敏感配置 | 4h |
| URL 白名单验证 | 限制 API URL 协议和域名 | 2h |
...
|------|------|
| 功能名称 | 漏洞验证辅助模块 |
| 解决问题 | AI 生成的 Payload 缺乏验证，误报率高 |
| 实现思路 | 为每种漏洞类型设计验证逻辑：SQL 注入验证盲注条件、XSS 验证回显位置、命令注入验证延迟/回显 |
| 实用价值 | 10/10 |

**验证策略设计**:
- SQL 注入: 布尔条件验证、时间延迟验证、错误信息验证
- XSS: 回显位置定位、DOM 执行环境检测
- 命令注入: 延迟检测、回显提取、OOB 检测
- 路径遍历: 文件存在性验证、内容特征匹配
- SSRF: 内网探测、协议验证

**新增文件**:
```
src/main/java/burp/validation/
├── VulnerabilityValidator.java     # 验证引擎
├── ValidationStrategy.java         # 验证策略接口
├── SQLInjectionValidator.java      # SQL 注入验证
├── XSSValidator.java               # XSS 验证
├── CommandInjectionValidator.java  # 命令注入验证
├── PathTraversalValidator.java     # 路径遍历验证
├── SSRFValidator.java              # SSRF 验证
...
|------|------|
| 功能名称 | 响应差异分析器 |
| 解决问题 | Fuzzing 结果需人工逐个检查响应 |
| 实现思路 | 对比正常请求与 Payload 请求的响应差异，标记异常响应 |
| 实用价值 | 9/10 |
...
**新增文件**:
```
src/main/java/burp/analyzer/
├── ResponseDiffAnalyzer.java    # 响应差异分析
├── DiffResult.java              # 差异结果
├── BaselineResponse.java        # 基线响应
...
| 功能名称 | 敏感信息自动识别 |
| 解决问题 | 响应中的敏感信息容易被忽略 |
| 实现思路 | 正则规则匹配敏感信息，自动高亮提取 |
| 实用价值 | 9/10 |

**识别规则**:
- API Key / Token (AWS, GitHub, JWT, etc.)
- 密码/凭证
- 身份证/手机号/银行卡
- 内网 IP / 私有域名
- 数据库连接串
...
**新增文件**:
```
src/main/java/burp/sensitive/
├── SensitiveDataDetector.java   # 敏感数据检测
├── SensitivePattern.java        # 敏感模式定义
├── SensitiveMatch.java          # 匹配结果
└── patterns.json                # 模式配置
```
...
**新增文件**:
```
src/main/java/burp/ui/
├── DashboardPanel.java          # 监控面板
└── components/
    ├── StatsChart.java          # 统计图表
    └── ProgressIndicator.java   # 进度指示器
```

---

#### 14.3.3 自定义 Payload 模板库 [P2]
...
- **核心功能**: 漏洞验证、一键工作流、响应分析、报告生成
- **效率提升**: 任务队列、敏感信息识别、智能推荐、知识库
